...
@section('content')
    ...
    <p class="text-gray-500 mt-1 mb-6">Meninjau weekly seluruh karyawan</p>
    ...
    <div class="flex gap-3 mb-8 flex-wrap">
        @forelse ($daftarUser as $u)
            <a href="{{ route('hrd.feedback.index', ['user' => $u->id, 'divisi_id' => request('divisi_id')]) }}"
               class="flex items-center gap-2 px-4 py-2 rounded-lg border {{ $pic && $pic->id === $u->id ? 'border-blue-400 bg-blue-50' : 'border-gray-200' }}">
                <span class="w-7 h-7 rounded-full bg-blue-700 text-white flex items-center justify-center text-xs font-semibold">
                    {{ strtoupper(substr($u->nama, 0, 2)) }}
                </span>
                <span class="text-sm text-gray-800">{{ $u->nama }}</span>
                <span class="text-xs text-gray-400">· {{ $u->divisi->nama ?? '-' }}</span>
            </a>
        @empty
            <p class="text-gray-400 text-sm">Tidak ada karyawan.</p>
        @endforelse
    </div>
    ...
    @else
        ...
        <form method="POST" action="{{ route('hrd.feedback.store', $pic) }}">
            @csrf
            <input type="hidden" name="weekly_commitment_id" value="{{ $commitment->id }}">
            <textarea name="comment" rows="4" required placeholder="Tulisan feedback dari HRD"
                      class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-blue-400"></textarea>
            <button type="submit" class="px-6 py-2.5 bg-blue-700 hover:bg-blue-800 text-white font-semibold rounded-lg text-sm transition">
                Kirim Komentar
            </button>
        </form>
    @endif
