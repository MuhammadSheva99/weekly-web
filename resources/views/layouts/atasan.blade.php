...
<head>
    ...
    <title>@yield('title', 'Head') - Weekly Performance Management</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="antialiased bg-gray-50">
    <div class="flex min-h-screen">

        <aside class="w-72 shrink-0 bg-red-700 text-white flex flex-col">
            <div class="px-8 py-10">
                <h1 class="text-2xl font-bold tracking-wide">INDOBOGA</h1>
                <p class="text-xs text-red-100 tracking-widest mt-1">PT INDOBOGA MAKMUR PRATAMA</p>
            </div>

            <nav class="flex-1 px-4 space-y-1 overflow-y-auto">
                @php
                    $menus = [
                        ['label' => 'Dashboard', 'route' => 'atasan.dashboard'],
                        ['label' => 'Weekly commitment', 'route' => null],
                        ['label' => 'Weekly progress', 'route' => null],
                        ['label' => 'Self review', 'route' => null],
                ...
                @foreach ($menus as $menu)
                    ...
                    <a href="{{ $menu['route'] ? route($menu['route']) : '#' }}"
                       class="block px-4 py-3 rounded-lg font-medium transition {{ $active ? 'bg-white text-red-700' : 'text-white/90 hover:bg-white/10' }}">
                        {{ $menu['label'] }}
                    </a>
                @endforeach
            </nav>
            ...
            <div class="px-6 py-6 border-t border-white/20 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-red-100">{{ auth()->user()->jabatan ?? auth()->user()->role->nama }}</p>
                </div>
            </div>
