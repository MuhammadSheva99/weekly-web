...
@section('content')
    ...
    <form method="POST" action="{{ route('karyawan.cuti.store') }}" enctype="multipart/form-data" class="space-y-6 max-w-xl">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="tanggal_mulai" id="tanggal_mulai" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm">
            </div>
            <div>
                <label class="block text-sm text-gray-600 mb-1">Tanggal selesai</label>
                <input type="date" name="tanggal_selesai" id="tanggal_selesai" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm">
            </div>
        </div>

        <div>
            <label class="block text-sm text-gray-600 mb-1">Jumlah hari (otomatis)</label>
            <input type="text" id="jumlah_hari_display" readonly disabled value="- hari"
                   class="w-full px-4 py-2.5 border border-gray-200 bg-gray-100 rounded-lg text-sm text-gray-500">
        </div>
        ...
        @if ($atasan)
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">Pengajuan akan diteruskan ke atasan langsung, lalu ke HRD untuk persetujuan akhir.</p>
            </div>
        @else
    ...
    <script>
        ...
        const display = document.getElementById('jumlah_hari_display');
        ...
        function hitungHari() {
            if (mulai.value && selesai.value) {
                ...
                display.value = diff > 0 ? diff + ' hari' : '- hari';
            }
        }
