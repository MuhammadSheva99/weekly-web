...
@section('content')
    ...
    <form method="POST" action="{{ route('karyawan.weekly-commitment.store') }}">
        ...
        <div class="border border-gray-200 rounded-xl overflow-hidden mb-8">
            ...
            @php
                $defaultMetrics = ['Metric 1', 'Metric 2', 'Metric 3', 'Metric 4', 'Target Utama'];
            @endphp
        ...
        <textarea name="output_deliverable" required rows="3"
                  class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm mb-8 focus:outline-none focus:ring-2 focus:ring-amber-400">{{ old('output_deliverable') }}</textarea>
