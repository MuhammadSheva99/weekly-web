...
@section('title', 'Izin Anggota Divisi')
    ...
    <div class="space-y-4">
        @forelse ($daftar as $izin)
            <div class="bg-white border border-gray-200 rounded-2xl p-5 flex items-center gap-6">
                ...
                <div class="flex flex-col gap-2 shrink-0">
                    <form method="POST" action="{{ route('atasan.izin-anggota-divisi.approve', $izin) }}">
                        @csrf
                    ...
                    <form method="POST" action="{{ route('atasan.izin-anggota-divisi.reject', $izin) }}" onsubmit="return confirm('Yakin tolak pengajuan ini?')">
                        @csrf
        ...
        @empty
            <div class="bg-white border border-gray-200 rounded-2xl p-10 text-center text-gray-400">
                Tidak ada pengajuan izin yang menunggu dari anggota tim kamu.
            </div>
        @endforelse
    </div>
