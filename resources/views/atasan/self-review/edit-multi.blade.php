...
@section('content')
    ...
    <form method="POST" action="{{ route('atasan.self-review.update') }}">
        @csrf
        @method('PUT')
        ...
        @foreach ($questions as $field => $label)
            ...
            <textarea name="{{ $field }}" rows="2" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm mb-6 focus:outline-none focus:ring-2 focus:ring-red-400">{{ old($field, $firstReview->$field) }}</textarea>
        @endforeach
