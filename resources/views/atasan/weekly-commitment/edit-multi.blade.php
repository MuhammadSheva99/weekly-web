...
@section('content')
    ...
    <form method="POST" action="{{ route('atasan.weekly-commitment.update', $first) }}">
        @csrf
        @method('PUT')
        ...
        <textarea name="output_deliverable" required rows="3"
                  class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm mb-8 focus:outline-none focus:ring-2 focus:ring-red-400">{{ old('output_deliverable', $first->output_deliverable) }}</textarea>
